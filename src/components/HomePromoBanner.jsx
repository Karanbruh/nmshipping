...
function HomePromoBanner({ variant }) {
  ...
  if (variant === 'cricket') {
    return <TournamentHighlightsPromo promo={promo} />
  }
...
function TournamentHighlightsPromo({ promo }) {
  ...
  const advance = useCallback(() => {
    setActiveIndex((prev) => (prev + 1) % TOURNAMENT_CAROUSEL_IMAGES.length)
  }, [])
          ...
          <div className="col-lg-6">
            <ScrollReveal variant="fade-left" delay={120}>
              <div
                ...
                aria-atomic="true"
              >
                {TOURNAMENT_CAROUSEL_IMAGES.map((image, index) => (
                  <img
                    key={image.src}
                    src={image.src}
                    alt={image.alt}
                ...
                <div className="home-promo-section__dots" role="tablist" aria-label="Tournament photos">
                  {TOURNAMENT_CAROUSEL_IMAGES.map((image, index) => (
                    <button
                      key={image.src}
                      type="button"
